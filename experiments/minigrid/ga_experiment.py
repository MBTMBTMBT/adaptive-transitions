...
from customised_minigrid_env.customised_minigrid_env import (
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="MiniGrid: GA → Curriculum → Visualization with W&B (images only)."
    )
    ...
    p.add_argument("--ga-add-edge-attempts-per-child", type=int, default=250)
    ...
    p.add_argument("--ga-prob-tweak-actions-per-child", type=int, default=500)
...
def main():
    ...
    args = resolve_args(
        parser
    )  # your resolver builds agent_kwargs, seeds, phase_steps, etc.

    # Validate map name early for clearer error messages
    if args.map not in set(list_builtin_maps()):
        raise ValueError(
            f"Unknown --map '{args.map}'. Available: {', '.join(list_builtin_maps())}"
        )
    ...
    if not args.skip_train and json_files:
        ...
        _ = stage_train(
            ...
            target_factory_kwargs=dict(
                map_name=args.map, max_steps=int(args.max_steps)
            ),
