...
#
# Layout assumptions (this file lives in ./experiments/simplegrid):
#   - script & ga_experiment.py:   ./experiments/simplegrid
#   - repo root:                   ./../..
...
#      bash experiments/simplegrid/ga_experiment.sh --exp ga-simplegrid --use-slurm
#
# 1) Local (Apptainer, default maps env0..env4; outputs -> ./experiment_output):
#      experiments/simplegrid/ga_experiment.sh --exp my_sg
#
# 2) Local with specific maps:
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --maps env2,env4
#
# 3) Read maps from file (one per line):
#      printf "env0\nenvX\n" > maps.txt
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --maps-file maps.txt
#
# 4) Force host venv (NO container):
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --no-container
#
# 5) Use a specific local container path:
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --container /abs/path/container.sif
#
# 6) Submit one SLURM job per map (Singularity; outputs -> /scratch/users/$USER/experiment_output):
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --use-slurm
#      # custom resources:
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --use-slurm \
...
#      experiments/simplegrid/ga_experiment.sh --exp my_sg -- --wandb-mode offline --skip-train
#
# 9) Select objective groups (CSV or file; default = ALL groups):
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --obj-groups perf_source_target,kl
#      experiments/simplegrid/ga_experiment.sh --exp my_sg --obj-groups-file groups.txt
#
# Output structure:
...
# Repo-aware paths
# -----------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"     # .../experiments/simplegrid
