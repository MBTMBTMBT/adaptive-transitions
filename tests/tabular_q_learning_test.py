...
if __name__ == '__main__':
    ...
    learning_rate = 0.5
    gamma = 0.99
    policy_mix = (0.6, 0.3, 0.1)
    temperature = 1.0

    seeds = [0, 1, 2]

    # --- Stochastic Taxi (rainy)
    print("\n=== Build stochastic Taxi (rainy) MDP and NetworkX backend ===")
    taxi_src_env = CustomisedTaxiEnv(render_mode=None, is_rainy=True)
    taxi_src_env.reset(seed=123)
    ...
    fl_src_env = CustomisedFrozenLakeEnv(render_mode=None, map_name="8x8", is_slippery=True)
    fl_src_env.reset(seed=999)
