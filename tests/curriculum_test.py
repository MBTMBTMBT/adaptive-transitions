...
LINE_CURR_EVAL_SOURCE = (
    "Curriculum (eval on Source-A)"  # same training, eval on Source-A
)
...
def build_checkpoints_from_curve_len(
    steps_a: int, steps_b: int, n_points: int
) -> np.ndarray:
    """
...
def _run_one_seed(seed: int, json_files: List[str]) -> Dict:
    ...
    base_agent.learn(
        total_timesteps=STEPS_JSON_PHASE,
        reset_num_timesteps=False,
        progress_bar=False,
        callback=base_cb,
    )
    base_agent.learn(
        total_timesteps=STEPS_BASE_PHASE,
        reset_num_timesteps=False,
        progress_bar=False,
        callback=base_cb,
    )
...
if __name__ == "__main__":
    ...
    for res in results:
        assert len(res["checkpoints"]) == len(
            checkpoints
    ...
    mixed_base_greedy_by_json: Dict[str, List[np.ndarray]] = {
        Path(p).stem: [] for p in json_paths
    }
    mixed_base_train_by_json: Dict[str, List[np.ndarray]] = {
        Path(p).stem: [] for p in json_paths
    }
    mixed_phase_greedy_by_json: Dict[str, List[np.ndarray]] = {
        Path(p).stem: [] for p in json_paths
    }
    mixed_phase_train_by_json: Dict[str, List[np.ndarray]] = {
        Path(p).stem: [] for p in json_paths
    }
    ...
    # =========================
    def _agg(
        dct_lists: Dict[str, List[np.ndarray]]
    ...
    for label in sorted(mixed_base_g.keys()):
        ...
        ax1.plot(
            checkpoints,
            base_greedy_mean,
            label=LINE_TARGET_BASELINE,
            linewidth=OTHER_LW,
        )
        ax1.fill_between(
            checkpoints,
            base_greedy_mean - base_greedy_std,
            base_greedy_mean + base_greedy_std,
            alpha=0.2,
        )
        ...
        # Lines (Train-policy)
        ax2.plot(
            checkpoints, base_train_mean, label=LINE_TARGET_BASELINE, linewidth=OTHER_LW
        )
        ax2.fill_between(
            checkpoints,
            base_train_mean - base_train_std,
            base_train_mean + base_train_std,
            alpha=0.2,
        )
        ...
        for ax in (ax1, ax2):
            ...
            ax.text(
                phase_boundary + (checkpoints[-1] - phase_boundary) * 0.5,
                ytxt,
                f"Phase B ({TARGET_NAME})",
                ha="center",
                va="bottom",
                fontsize=9,
                alpha=0.8,
    ...
    for label in sorted(mixed_base_g.keys()):
        print(f"[{label}]")
        print(
            f"  {LINE_TARGET_BASELINE} — Greedy: {base_greedy_mean[-1]:.3f} ± {base_greedy_std[-1]:.3f} | "
            f"TrainPol: {base_train_mean[-1]:.3f} ± {base_train_std[-1]:.3f}"
        )
        print(
            f"  {LINE_CURR_EVAL_TARGET} — Greedy: {mixed_base_g[label]['mean'][-1]:.3f} ± {mixed_base_g[label]['std'][-1]:.3f} | "
            f"TrainPol: {mixed_base_t[label]['mean'][-1]:.3f} ± {mixed_base_t[label]['std'][-1]:.3f}"
        )
        print(
            f"  {LINE_CURR_EVAL_SOURCE} — Greedy: {mixed_phase_g[label]['mean'][-1]:.3f} ± {mixed_phase_g[label]['std'][-1]:.3f} | "
