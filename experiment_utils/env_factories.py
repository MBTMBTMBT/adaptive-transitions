...
class IntegerStateObsWrapper(gym.ObservationWrapper):
    ...
    def reset(self, **kwargs) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        if self.keep_raw_obs_in_info:
            info = dict(info or {}); info["raw_obs"] = raw_obs
        return obs, info
...
def make_frozenlake(seed: int, cfg: Dict[str, Any]):
    ...
    env = CustomisedFrozenLakeEnv(map_name=map_name, is_slippery=is_slippery,
                                  networkx_env=None, render_mode="rgb_array")
...
def make_taxi(seed: int, cfg: Dict[str, Any]):
    ...
    env = CustomisedTaxiEnv(is_rainy=False, fickle_passenger=False,
                            networkx_env=None, render_mode="rgb_array")
    env = TimeLimit(env, max_episode_steps=max_steps)
    if seed is not None: env.reset(seed=seed)
    return env
...
def make_minigrid(seed: int, cfg: Dict[str, Any]):
    ...
    env = CustomMiniGridEnv(
        map_name=map_name, random_rotate=False, random_flip=False,
        display_mode="middle", any_key_opens_the_door=False,
        networkx_env=None, render_mode="rgb_array",
    )
...
def make_nx_env_from_mdp(seed: int, cfg: Dict[str, Any]):
    ...
    max_steps = int(cfg.get("max_steps", 500))
    mdp_cfg = cfg.get("mdp_config_path", None)
    # if mdp_cfg is None:
    #     raise ValueError("cfg['mdp_config_path'] is required for make_nx_env_from_mdp")
    mdp = MDPNetwork(config_path=mdp_cfg)
